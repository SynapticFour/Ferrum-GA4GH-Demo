...
def _publication_block(
    ...
    lines.append(
        ...
        f"requests per mode (plain; capped at **{max_b_disp}** per request unless `--max-bytes 0`). "
        f"See `scripts/drs_micro_benchmark.py` (`--repeat`).\n\n"
    )
...
5. **DRS micro** — `scripts/drs_micro_benchmark.py` times `GET .../objects/{{id}}/stream` (**n = {drs_n}** runs per mode by default); optional `X-Crypt4GH-Public-Key`; see `results/drs_micro.json`.
