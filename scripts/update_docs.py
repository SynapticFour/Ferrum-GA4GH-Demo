...
def generate_africa_section(root: Path) -> str:
    """Africa resilience feature table for docs/benchmark.md."""
    africa = _load_json(root / "results" / "africa_results.json") or {}
    ...
    rows = [
        ...
        ("ONT/Nanopore Ingestion", "ont_ingestion"),
        ("Multi-Pathogen Beacon", "multi_pathogen_beacon"),
        ("Outbreak Mode", "outbreak_mode"),
        ("Data Residency Audit", "residency_audit"),
...
def _publication_block(
    ...
    # Structured DRS micro summary for reviewers (keys in results/drs_micro.json)
    def _med(d: dict, path: str) -> str:
        cur = d
        for k in path.split("."):
            if not isinstance(cur, dict):
                return "n/a"
            cur = cur.get(k) or {}
        if isinstance(cur, dict) and cur.get("median") is not None:
            return str(cur["median"])
        return "n/a"

    plain_m = _med(dm, "plain.wall_seconds")
    ar_m = _med(dm, "crypt4gh_at_rest.wall_seconds")
    hdr_m = _med(dm, "crypt4gh.wall_seconds")
    enc_oid = dm.get("encrypted_object_id")
    lines.append("**DRS micro JSON keys** (`results/drs_micro.json`):\n\n")
    lines.append(
    ...
    lines.append("**DRS micro medians (wall time, seconds):**\n\n")
    lines.append("| Mode | Median (s) |\n|------|------------|\n")
    lines.append(f"| Plain `ref_fasta` | {plain_m} |\n")
    if has_at_rest:
        lines.append(f"| Crypt4GH **at-rest** (server decrypt) | {ar_m} |\n")
    hdr_ok = isinstance(dm.get("crypt4gh"), dict) and not (dm.get("crypt4gh") or {}).get(
        "skipped"
    )
    if hdr_ok and hdr_m != "n/a":
        lines.append(f"| Client header `X-Crypt4GH-Public-Key` | {hdr_m} |\n")
    lines.append("\n")
    if enc_oid:
        lines.append(
            f"**Encrypted `ref_fasta` object id** (at-rest leg): `{enc_oid}` "
            f"(plain-phase mapping snapshot: `results/drs_mapping_phase_plain.json` after `./run --macro`).\n\n"
        )
...
def main() -> None:
    ...
    crypt_hdr_med = None
    if isinstance(crypt, dict) and not crypt.get("skipped"):
        cw = crypt.get("wall_seconds") or {}
        if isinstance(cw, dict):
            crypt_hdr_med = cw.get("median")
...
"""
    ...
    hdr_cell = (
        str(crypt_hdr_med)
        if crypt_hdr_med is not None
        else "n/a (set `FERRUM_GA4GH_CRYPT4GH_PUBKEY` for header leg)"
    )
...
| DRS stream Crypt4GH **at-rest** (median s, server decrypt) | {at_rest_med if at_rest_med is not None else "n/a (`./run --macro` merges this)"} |
| DRS stream client header `X-Crypt4GH-Public-Key` (median s) | {hdr_cell} |
| DRS micro repetitions (n) | {drs_n} |
